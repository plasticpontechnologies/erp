	...
	public List<MessSchedule>  getMenuData() {
	  ...
	  @PostMapping(value = "/saveMessSchedule", consumes = "application/json")
		public void saveMessSchedule(@RequestBody MessSchedule canteen) {
		  ...
		  @GetMapping(value="/get/{dId}/{cId}")
		  public List<MessSchedule> getDayAndMealType(@PathVariable int dId,@PathVariable int cId) {
			 return  messService.findByDayAndMealType(dId,cId);
		  }
